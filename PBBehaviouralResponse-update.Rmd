...
So we've got 2,406 observations to work with, a reaction rate of around 24.6%, and distances ranging from 1 m right out to nearly 9 km — with a median of 200 m. 
...
The model selection table provides strong support for a single top model: a spline-based distance term interacting with activity class, combined with season, group composition, and survey method as additional covariates (spl_survey). No other model comes within 8 AICc units of the top model, and the ΔAICc < 2 threshold is met by this model alone (this is the general rule for model selection using AIC - if a simpler model reduces the AIC by 2 or more, thewn it is favourable or "more parismonius" in the Okkams Razxor sense. The clear winner here is both the flexible spline distance form and the inclusion of survey method as a covariate — the equivalent log-distance model with survey method (log_survey) is the next best but sits nearly 9 units behind. This gives confidence that the selected model structure is appropriate and that model-averaging or multi-model inference is not required here.
...
The highest-ranked model is used for all subsequent inference and prediction. Its coefficient estimates, standard errors, and significance values are shown below. The model summary reveals a strong and highly significant effect of distance on reaction probability, captured across the spline terms. Three of the four spline basis coefficients are statistically significant (p < 0.01), confirming that the distance–response relationship is real and robust, and the overall reduction in deviance from the null model (2,600.6) to the fitted model (2,237.6) — a reduction of approximately 363 units on 15 degrees of freedom — reflects a substantial improvement in fit. In contrast, the individual main effects of activity class, season, group composition, and survey method are not statistically significant on their own, though the interaction between distance and activity class is significant at the first spline knot (p = 0.002), indicating that the shape of the distance–response relationship does differ meaningfully between stationary and mobile activities at closer ranges (we can chat about knots and their meaning some more during the meeting). The large standard error and near-zero coefficient for hovercraft survey method reflects very few observations from that platform and should be interpreted with caution; it does not affect predictions made under the other survey method levels.

I understand it can be confusing that there are non-significant model covariates that are retained through the model selection process but we can discuss this during the meeting.
...
Predicted reaction probabilities are extracted at two distances of direct operational relevance: 805 m and 1,610 m. These are reported alongside 95% confidence intervals under the same reference covariate settings used for the prediction curves above (open water season, no cubs present, stationary activity class as baseline).

At 805 m, the model predicts a reaction probability of 4.3% under reference conditions (open water, no cubs, stationary activity), which falls just below the 5% threshold that is often used as a practical lower bound for risk. At 1,610 m, the predicted probability falls slightly further to 3.5%. While the point estimates suggest that reaction risk is low at both distances, the confidence intervals are notably wide, spanning from below 1% to approximately 25–29% at both distances. This reflects genuine uncertainty in the predicted probabilities at these specific distances, stemming from the flexible spline model form and the relatively sparse data at larger distances. These wide intervals should be communicated clearly when these figures are used to inform management decisions, and point estimates alone should not be cited without their associated uncertainty.
...
In addition to predicted probabilities at fixed distances, the analysis estimates the distance at which the predicted reaction probability drops to each of two target thresholds: 5% and 10%. These thresholds represent residual risk levels that may be relevant for setting operational exclusion zones or activity buffers.

Point estimates are obtained by searching along the predicted distance–response curve for the crossing point using a fine distance grid. Uncertainty is quantified via a parametric bootstrap: 1,000 sets of model coefficients are drawn from the multivariate normal distribution defined by the fitted coefficients and their estimated covariance matrix. The threshold distance is recalculated for each draw, and the 2.5th and 97.5th percentiles of the resulting distribution form the 95% confidence interval. Thresholds are reported separately for stationary and mobile activity classes, with all other covariates held at reference levels.

The estimated threshold distances at which the predicted reaction probability drops to 5% or 10% carry very wide confidence intervals, and for mobile activity at the 5% threshold no finite point estimate could be obtained within the observed distance range. For stationary activity, the 5% threshold is estimated at approximately 398 m (95% CI: 32–9,403 m), and for mobile activity the 10% threshold is estimated at 1,349 m (95% CI: 1–5,583 m). The extreme width of these intervals reflects two things: the inherent uncertainty in the spline model at the tails of the distance distribution, and the fact that the predicted distance–response curve for mobile activity does not decline as steeply or as smoothly as for stationary activity, making the crossing of a specific probability threshold less precisely defined. These results suggest that while distance is clearly an important predictor of reaction probability, the data do not support precise threshold recommendations at this stage. I would suggest that any management distances derived from these estimates should be treated as indicative rather than definitive, and the confidence intervals should feature VERY prominently in any  communication.
...
```{r thresholds, cache=TRUE, echo=FALSE}
